...
$ADB shell "sqlite3 /data/data/org.blissroms.updater/databases/updates.db" \
    "\"INSERT INTO updates (status, path, download_id, timestamp, version, size, name, os_patch_level, os_sdk_level)" \
    "  VALUES ($status, '$zip_path_device', '$id', $timestamp, '$version', $size, '$zip_name', '$os_patch_level', $os_sdk_level)\""
